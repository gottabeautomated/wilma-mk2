...
import { PrioritiesData } from '../../types/budget'
import ElegantIcon from '../ElegantIcon'

interface PrioritiesFormProps {
  data: Partial<PrioritiesData>
  errors: Partial<Record<keyof PrioritiesData, string>>
  onChange: (data: Partial<PrioritiesData>) => void
}

const PrioritiesForm: React.FC<PrioritiesFormProps> = ({ data, errors, onChange }) => {
  const [newMustHave, setNewMustHave] = useState('')
  const [newNiceToHave, setNewNiceToHave] = useState('')
  const [newDealBreaker, setNewDealBreaker] = useState('')

  const updateField = (field: keyof PrioritiesData, value: any) => {
  ...
  const toggleArrayField = (field: keyof PrioritiesData, value: string) => {
  ...
  const addToList = (field: keyof PrioritiesData, value: string, setter: (value: string) => void) => {
  ...
  const removeFromList = (field: keyof PrioritiesData, value: string) => {
    const currentArray = (data[field] as string[]) || []
  ...
  const getPriorityColor = (value: number) => {
    ...
    if (value >= 6) return 'bg-champagne'
  ...
  const getPriorityLabel = (value: number) => {
    if (value >= 9) return 'Absolut wichtig'
    if (value >= 7) return 'Sehr wichtig'
    if (value >= 5) return 'Wichtig'
    if (value >= 3) return 'Mäßig wichtig'
    if (value >= 1) return 'Weniger wichtig'
    return 'Unwichtig'
  }
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {priorityCategories.map((category, index) => {
              const currentValue = data.topPriorities?.[category.id as keyof typeof data.topPriorities] || 5
              return (
                <motion.div
                  key={category.id}
                  ...
                  className="card-elegant hover:shadow-golden transition-all duration-300"
                >
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="relative">
                      <div className="relative h-3 bg-gradient-to-r from-primary-100 to-undertone rounded-elegant overflow-hidden shadow-inner">
                        <motion.div
                          className={`h-full rounded-elegant ${getPriorityColor(currentValue)}`}
                      ...
                      <input
                        ...
                        max="10"
                        value={currentValue}
                        onChange={(e) => updatePriority(category.id, parseInt(e.target.value))}
        ...
        >
          ...
          <div className="flex gap-3 mb-6">
            <input
              ...
              placeholder="z.B. Live-Musik während der Zeremonie"
              className="flex-1 px-4 py-3 border-2 border-undertone rounded-elegant focus:border-accent focus:ring-0 focus:outline-none transition-all duration-300 text-graphite placeholder-accent/50 font-light"
              onKeyPress={(e) => e.key === 'Enter' && addToList('mustHaves', newMustHave, setNewMustHave)}
            />
            <motion.button
              onClick={() => addToList('mustHaves', newMustHave, setNewMustHave)}
              className="px-6 py-3 bg-accent text-white rounded-elegant hover:bg-primary-600 transition-colors font-medium shadow-elegant"
          ...
          <div className="flex flex-wrap gap-3">
            {(data.mustHaves || []).map((item, index) => (
              <motion.span
                key={index}
                initial={{ opacity: 0, scale: 0.8 }}
                animate={{ opacity: 1, scale: 1 }}
                transition={{ delay: 0.1 * index }}
                className="inline-flex items-center px-4 py-2 rounded-elegant text-sm bg-primary-100 text-graphite border border-accent/20 shadow-sm"
              >
                <ElegantIcon name="heart" className="text-accent mr-2" size={14} />
                {item}
                <button
                  onClick={() => removeFromList('mustHaves', item)}
        ...
        >
          ...
          <div className="flex gap-3 mb-6">
            <input
              ...
              placeholder="z.B. Feuerwerk zum Abschluss"
              className="flex-1 px-4 py-3 border-2 border-undertone rounded-elegant focus:border-accent focus:ring-0 focus:outline-none transition-all duration-300 text-graphite placeholder-accent/50 font-light"
              onKeyPress={(e) => e.key === 'Enter' && addToList('niceToHaves', newNiceToHave, setNewNiceToHave)}
            />
            <motion.button
              onClick={() => addToList('niceToHaves', newNiceToHave, setNewNiceToHave)}
              className="px-6 py-3 bg-champagne text-graphite rounded-elegant hover:bg-primary-300 transition-colors font-medium shadow-elegant"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              Hinzufügen
          ...
          <div className="flex flex-wrap gap-3">
            {(data.niceToHaves || []).map((item, index) => (
              <motion.span
                key={index}
                initial={{ opacity: 0, scale: 0.8 }}
                animate={{ opacity: 1, scale: 1 }}
                transition={{ delay: 0.1 * index }}
                className="inline-flex items-center px-4 py-2 rounded-elegant text-sm bg-champagne/30 text-graphite border border-champagne/40 shadow-sm"
              >
                <ElegantIcon name="flower" className="text-accent mr-2" size={14} />
                {item}
                <button
                  onClick={() => removeFromList('niceToHaves', item)}
                  className="ml-2 text-accent/60 hover:text-accent transition-colors"
                >
                  <ElegantIcon name="calendar" size={12} />
                </button>
        ...
        >
          ...
          <div className="flex gap-3 mb-6">
            <input
              ...
              placeholder="z.B. Regen während der Outdoor-Zeremonie"
              className="flex-1 px-4 py-3 border-2 border-undertone rounded-elegant focus:border-accent focus:ring-0 focus:outline-none transition-all duration-300 text-graphite placeholder-accent/50 font-light"
              onKeyPress={(e) => e.key === 'Enter' && addToList('dealBreakers', newDealBreaker, setNewDealBreaker)}
            />
            <motion.button
              onClick={() => addToList('dealBreakers', newDealBreaker, setNewDealBreaker)}
              className="px-6 py-3 bg-red-400 text-white rounded-elegant hover:bg-red-500 transition-colors font-medium shadow-elegant"
              whileHover={{ scale: 1.02 }}
              whileTap={{ scale: 0.98 }}
            >
              Hinzufügen
          ...
          <div className="flex flex-wrap gap-3">
            {(data.dealBreakers || []).map((item, index) => (
              <motion.span
                key={index}
                initial={{ opacity: 0, scale: 0.8 }}
                animate={{ opacity: 1, scale: 1 }}
                transition={{ delay: 0.1 * index }}
                className="inline-flex items-center px-4 py-2 rounded-elegant text-sm bg-red-50 text-red-700 border border-red-200 shadow-sm"
              >
                <ElegantIcon name="castle" className="text-red-500 mr-2" size={14} />
                {item}
                <button
                  onClick={() => removeFromList('dealBreakers', item)}
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {inspirationSources.map((source, index) => (
              <motion.button
                key={source.id}
                onClick={() => toggleArrayField('inspirationSources', source.id)}
                className={`card-elegant text-center transition-all duration-300 ${
                  data.inspirationSources?.includes(source.id as any)
                    ? 'border-2 border-gold bg-gold-light/10 shadow-gold'
                    : 'hover:shadow-gold hover:border-gold/30'
                }`}
                initial={{ opacity: 0, scale: 0.8 }}
                animate={{ opacity: 1, scale: 1 }}
                transition={{ delay: 0.1 * index }}
              ...
              >
                <ElegantIcon 
                  ...
                  className={`mx-auto mb-3 ${
                    data.inspirationSources?.includes(source.id as any) 
                      ? 'text-gold' 
                      : 'text-accent/60 hover:text-gold'
