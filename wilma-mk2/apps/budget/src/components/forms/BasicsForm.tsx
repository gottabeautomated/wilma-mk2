...
import { BasicsData, ValidationErrors } from '../../types/budget'
import ElegantIcon from '../ElegantIcon'

interface BasicsFormProps {
  data: Partial<BasicsData>
  errors: Partial<Record<keyof BasicsData, string>>
  onChange: (data: Partial<BasicsData>) => void
}

const BasicsForm: React.FC<BasicsFormProps> = ({ data, errors, onChange }) => {
  const updateField = (field: keyof BasicsData, value: any) => {
      ...
      <div className="space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            >
              ...
              <input
                ...
                data-field="partner1Name"
                className={`w-full px-4 py-3 border-2 rounded-elegant transition-all duration-300 text-graphite placeholder-accent/50 font-light ${
                  errors.partner1Name
            ...
            >
              ...
              <input
                ...
                placeholder="Name Ihres Partners"
                data-field="partner2Name"
                className={`w-full px-4 py-3 border-2 rounded-elegant transition-all duration-300 text-graphite placeholder-accent/50 font-light ${
                  errors.partner2Name
                    ? 'border-red-400 focus:border-red-500'
                    : 'border-undertone focus:border-accent focus:ring-0 focus:outline-none'
        ...
        >
          ...
          <div className="max-w-md mx-auto">
            <input
              type="date"
              value={data.weddingDate ? formatDate(data.weddingDate) : ''}
              onChange={(e) => updateField('weddingDate', new Date(e.target.value))}
              min={new Date().toISOString().split('T')[0]}
              data-field="weddingDate"
              className={`w-full px-4 py-3 border-2 rounded-elegant transition-all duration-300 text-graphite font-light ${
                errors.weddingDate
                  ? 'border-red-400 focus:border-red-500'
                  : 'border-undertone focus:border-accent focus:ring-0 focus:outline-none'
              }`}
        ...
        >
          ...
          <div className="text-center mb-8">
            ...
            <motion.div
              className="inline-flex items-center px-4 py-2 rounded-elegant text-sm font-medium bg-primary-100 text-accent shadow-elegant"
          ...
          <div className="relative px-4 mb-8">
            <div className="relative h-4 bg-gradient-to-r from-primary-100 to-undertone rounded-elegant-lg overflow-hidden shadow-inner">
              <motion.div
                className="h-full bg-gradient-to-r from-champagne via-accent to-primary-500 rounded-elegant-lg"
                style={{ width: `${((data.guestCount || 80) - 20) / (300 - 20) * 100}%` }}
                initial={{ width: 0 }}
                animate={{ width: `${((data.guestCount || 80) - 20) / (300 - 20) * 100}%` }}
          ...
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {guestPresets.map((preset, index) => (
              <motion.button
                key={preset.count}
                onClick={() => updateField('guestCount', preset.count)}
                className={`card-elegant text-center transition-all duration-300 ${
                  data.guestCount === preset.count
              ...
              >
                <ElegantIcon 
                  ...
                  className={`mx-auto mb-2 ${
                    data.guestCount === preset.count ? 'text-accent' : 'text-accent/60'
                  }`} 
                  size={20} 
                />
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {relationshipLengths.map((option, index) => (
              <motion.button
                key={option.value}
                onClick={() => updateField('relationshipLength', option.value)}
                className={`card-elegant text-center transition-all duration-300 ${
                  String(data.relationshipLength) === option.value
                    ? 'border-2 border-accent bg-primary-100 shadow-golden'
                    : 'hover:border-accent/50 hover:bg-primary-50'
                }`}
              ...
              >
                <ElegantIcon 
                  name={option.icon} 
                  className={`mx-auto mb-2 ${
                    String(data.relationshipLength) === option.value ? 'text-accent' : 'text-accent/60'
                  }`} 
                  size={24} 
                />
