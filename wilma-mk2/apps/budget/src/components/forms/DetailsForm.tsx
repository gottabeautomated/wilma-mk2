...
import { DetailsData } from '../../types/budget'
import BudgetSlider from '../BudgetSlider'
import ElegantIcon from '../ElegantIcon'

interface DetailsFormProps {
  data: Partial<DetailsData>
  errors: Partial<Record<keyof DetailsData, string>>
  onChange: (data: Partial<DetailsData>) => void
}

const DetailsForm: React.FC<DetailsFormProps> = ({ data, errors, onChange }) => {
  const updateField = (field: keyof DetailsData, value: any) => {
  ...
  const toggleBudgetSource = (source: string) => {
  ...
  const togglePriority = (priorityId: string) => {
    ...
    const isSelected = currentPriorities.includes(priorityId as any)
    ...
    if (isSelected) {
      ...
      // Add priority (max 5)
      const newPriorities = [...currentPriorities, priorityId as any]
      updateField('topPriorities', newPriorities)
    }
  }
      ...
      <div className="space-y-8">
        ...
        >
          ...
          <div className="max-w-md mx-auto">
            <input
              ...
              placeholder="z.B. Wien, Salzburg, oder spezifische Location"
              className="w-full px-4 py-3 border-2 border-undertone rounded-elegant focus:border-accent focus:ring-0 focus:outline-none transition-all duration-300 text-graphite placeholder-accent/50 font-light"
            />
              ...
              >
                <ElegantIcon 
                  ...
                  className={`mx-auto mb-3 ${
                    data.venueType === venue.id ? 'text-accent' : 'text-accent/60'
                  }`} 
                  size={32} 
                />
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {budgetFlexibilityOptions.map((option, index) => (
              <motion.button
                key={option.id}
                onClick={() => updateField('budgetFlexibility', option.id)}
                className={`card-elegant text-center transition-all duration-300 ${
                  data.budgetFlexibility === option.id
                    ? 'border-2 border-accent bg-primary-100 shadow-golden'
                    : 'hover:border-accent/50 hover:bg-primary-50'
                }`}
                initial={{ opacity: 0, y: 20 }}
              ...
              >
                <ElegantIcon 
                  name={option.icon} 
                  className={`mx-auto mb-3 ${
                    data.budgetFlexibility === option.id ? 'text-accent' : 'text-accent/60'
                  }`} 
                  size={28} 
                />
                <h4 className="font-serif font-semibold text-graphite mb-2">
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {budgetSources.map((source, index) => (
              <motion.button
                key={source.id}
                onClick={() => toggleBudgetSource(source.id)}
                className={`card-elegant text-center transition-all duration-300 ${
                  data.budgetSource?.includes(source.id as any)
                    ? 'border-2 border-accent bg-primary-100 shadow-golden'
                    : 'hover:border-accent/50 hover:bg-primary-50'
                }`}
              ...
              >
                <ElegantIcon 
                  ...
                  className={`mx-auto mb-2 ${
                    data.budgetSource?.includes(source.id as any) ? 'text-accent' : 'text-accent/60'
                  }`} 
                  size={24} 
                />
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4">
            {priorityOptions.map((priority, index) => (
              <motion.button
                ...
                disabled={!data.topPriorities?.includes(priority.id as any) && (data.topPriorities?.length || 0) >= 5}
                className={`card-elegant text-center transition-all duration-300 relative ${
                  data.topPriorities?.includes(priority.id as any)
                    ? 'border-2 border-accent bg-primary-100 shadow-golden'
                ...
                whileHover={
                  !data.topPriorities?.includes(priority.id as any) && (data.topPriorities?.length || 0) < 5
                    ? { scale: 1.05, y: -2 }
                    : {}
                }
                whileTap={{ scale: 0.95 }}
              >
                <ElegantIcon 
                  name={priority.icon} 
                  className={`mx-auto mb-2 ${
                    data.topPriorities?.includes(priority.id as any) ? 'text-accent' : 'text-accent/60'
                  }`} 
                  size={24} 
                />
