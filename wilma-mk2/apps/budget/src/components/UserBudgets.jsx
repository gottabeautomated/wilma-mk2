...
const UserBudgets = ({ userEmail }) => {
    ...
    if (loading) {
        return (<div className="card-elegant">
    ...
    if (budgets.length === 0) {
        return (<div className="card-elegant text-center">
    ...
    return (<div className="space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {budgets.map((budget, index) => (<motion.div key={budget.id} className="card-elegant hover:shadow-royal cursor-pointer transition-all duration-300" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} onClick={() => setSelectedBudget(budget)}>
            <div className="flex items-start justify-between mb-4">
              <div className="flex items-center space-x-3">
                <div className="w-12 h-12 bg-gradient-to-br from-royal to-royal-dark rounded-elegant flex items-center justify-center">
                  <ElegantIcon name={getStyleIcon(budget.wedding_style)} className="text-white" size={20}/>
                </div>
                <div>
      ...
      {selectedBudget && (<motion.div className="fixed inset-0 bg-graphite/50 backdrop-blur-sm z-50 flex items-center justify-center p-4" initial={{ opacity: 0 }} animate={{ opacity: 1 }} onClick={() => setSelectedBudget(null)}>
          <motion.div className="card-elegant max-w-md w-full max-h-[80vh] overflow-y-auto" initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-serif font-bold text-graphite">
                Budget Details
              </h3>
              <button onClick={() => setSelectedBudget(null)} className="w-8 h-8 bg-undertone hover:bg-royal/20 rounded-elegant flex items-center justify-center transition-colors">
