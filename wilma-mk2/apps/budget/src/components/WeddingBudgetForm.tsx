...
import { WeddingFormData, BudgetData } from '../types/budget'
...
interface WeddingBudgetFormProps {
  onFormComplete?: (data: BudgetData) => void
}

const WeddingBudgetForm: React.FC<WeddingBudgetFormProps> = ({ onFormComplete }) => {
  const {
    currentStep,
    formData,
    validationErrors,
  ...
  const handleStepChange = (data: Partial<WeddingFormData[keyof WeddingFormData]>) => {
  ...
  const convertFormDataToBudgetData = (formData: WeddingFormData): BudgetData => {
    ...
    return (
      <div className="min-h-screen bg-gray-50">
        <BudgetResults 
          ...
          budgetData={convertFormDataToBudgetData(formData)}
        />
        {toast && (
          <Toast 
      ...
      <div className="relative z-10 container mx-auto px-4 py-8">
        ...
        >
          ...
          <motion.button
            ...
            className={`px-8 py-4 rounded-elegant font-medium transition-all ${
              currentStep === 1
                ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                : 'bg-white text-graphite border-2 border-undertone hover:border-accent hover:bg-primary-100'
            }`}
          ...
          {currentStep < FORM_STEPS.length ? (
            <motion.button
              onClick={nextStep}
              className="px-8 py-4 bg-royal text-white rounded-elegant font-medium transition-all shadow-royal hover:bg-royal-dark hover:shadow-royal-lg"
            ...
            <motion.button
              ...
              className={`px-8 py-4 rounded-elegant font-medium transition-all shadow-elegant ${
                isCalculating
                  ? 'bg-gray-400 text-white cursor-not-allowed'
                  : 'bg-royal text-white hover:bg-royal-dark hover:shadow-royal-lg'
              }`}
        ...
        >
          <div className="section-elegant">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {getCurrentStepTips().map((tip, index) => (
                <motion.div
                  key={index}
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
        ...
        <motion.div
          ...
          animate={{ opacity: 1 }}
          transition={{ delay: 1 }}
          className="text-center mt-8 text-sm text-accent/60"
        >
