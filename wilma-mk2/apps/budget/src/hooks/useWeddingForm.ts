...
import { WeddingFormData, ValidationErrors, AnalyticsEvents, LeadScore } from '../types/budget'
...
export const useWeddingForm = () => {
  ...
  const [analytics, setAnalytics] = useState<Partial<AnalyticsEvents>>({})
  ...
  // Analytics tracking
  const trackEvent = useCallback((eventName: keyof AnalyticsEvents, data: any) => {
    setAnalytics(prev => ({
      ...prev,
      [eventName]: data
    }))
  ...
  const updateStepData = useCallback((step: keyof WeddingFormData, data: any) => {
    setFormData(prev => ({
      ...prev,
      [step]: { ...prev[step], ...data }
    }))
  ...
  const validateCurrentStep = useCallback((): boolean => {
    ...
    switch (currentStep) {
      case 1: // Basics
        if (!stepData?.partner1Name) errors.partner1Name = 'Partner 1 Name ist erforderlich'
        if (!stepData?.partner2Name) errors.partner2Name = 'Partner 2 Name ist erforderlich'
        if (!stepData?.weddingDate) errors.weddingDate = 'Hochzeitsdatum ist erforderlich'
        if (!stepData?.guestCount || stepData.guestCount < 20) errors.guestCount = 'Mindestens 20 Gäste'
        if (!stepData?.email) errors.email = 'E-Mail ist erforderlich'
      ...
      case 2: // Details
        if (!stepData?.totalBudget || stepData.totalBudget < 5000) errors.totalBudget = 'Mindestbudget: €5.000'
        if (!stepData?.weddingLocation) errors.weddingLocation = 'Location ist erforderlich'
        if (!stepData?.venueType) errors.venueType = 'Venue-Typ ist erforderlich'
        if (!stepData?.budgetFlexibility) errors.budgetFlexibility = 'Budget-Flexibilität ist erforderlich'
        break

      case 3: // Style
        if (!stepData?.weddingStyle) errors.weddingStyle = 'Hochzeitsstil ist erforderlich'
        if (!stepData?.season) errors.season = 'Jahreszeit ist erforderlich'
        if (!stepData?.timeOfDay) errors.timeOfDay = 'Tageszeit ist erforderlich'
        if (!stepData?.formalityLevel) errors.formalityLevel = 'Formalitätslevel ist erforderlich'
        break

      case 4: // Priorities
        if (!stepData?.topPriorities) errors.topPriorities = 'Prioritäten sind erforderlich'
        break
    }
  ...
  const scrollToFirstError = useCallback((stepKey: keyof WeddingFormData, errors: Record<string, string>) => {
    ...
    setTimeout(() => {
      ...
      if (firstErrorField === 'totalBudget') {
        targetElement = document.querySelector('.budget-slider') as HTMLElement
      }

      // Try to find error message if no specific field found
      if (!targetElement) {
        targetElement = document.querySelector('.text-red-500') as HTMLElement
      }

      // If no error message, try to find the form section
      if (!targetElement) {
        targetElement = document.querySelector('.section-elegant') as HTMLElement
      }
      ...
      if (targetElement) {
        ...
        if (targetElement.classList.contains('text-red-500')) {
          const parentSection = targetElement.closest('.section-elegant') as HTMLElement
          if (parentSection) {
            parentSection.style.animation = 'gentle-shake 0.5s ease-in-out'
            setTimeout(() => {
              if (parentSection) parentSection.style.animation = ''
            }, 500)
          }
          ...
          setTimeout(() => {
            if (targetElement) targetElement.style.animation = ''
          }, 500)
        }
  ...
  const calculateLeadScore = useCallback((): LeadScore => {
    const { basics, details, style, priorities } = formData

    // Budget Range Score (1-10)
    let budgetRange = 1
    if (details?.totalBudget) {
      if (details.totalBudget >= 50000) budgetRange = 10
      else if (details.totalBudget >= 30000) budgetRange = 8
      else if (details.totalBudget >= 20000) budgetRange = 6
      else if (details.totalBudget >= 15000) budgetRange = 4
      else if (details.totalBudget >= 10000) budgetRange = 2
    }

    // Timeline Score (1-10) - näher = höher
    let timeline = 1
    if (basics?.weddingDate) {
      const monthsUntilWedding = Math.round(
        (new Date(basics.weddingDate).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24 * 30)
      )
      if (monthsUntilWedding <= 6) timeline = 10
      else if (monthsUntilWedding <= 12) timeline = 8
      else if (monthsUntilWedding <= 18) timeline = 6
      else if (monthsUntilWedding <= 24) timeline = 4
      else timeline = 2
    }
    ...
    let dataQuality = 0
    if (basics?.partner1Name) dataQuality += 1
    if (basics?.partner2Name) dataQuality += 1
    if (basics?.email) dataQuality += 1
    if (basics?.phone) dataQuality += 1
    if (details?.weddingLocation) dataQuality += 1
    if (details?.budgetSource?.length) dataQuality += 1
    if (style?.colorScheme?.length) dataQuality += 1
    if (style?.culturalTraditions?.length) dataQuality += 1
    if (priorities?.mustHaves?.length) dataQuality += 1
    if (priorities?.inspirationSources?.length) dataQuality += 1
...
const getStepKey = (step: number): keyof WeddingFormData => {
  switch (step) {
