  ...
  const filteredGuests = useMemo(() => {
    return guests.filter(guest => {
      ...
      if (config.filters.tableAssigned !== 'all') {
        if (config.filters.tableAssigned === 'assigned' && !guest.table_assignment_id) return false
        if (config.filters.tableAssigned === 'unassigned' && guest.table_assignment_id) return false
      }

      // Plus One filter
      if (config.filters.plusOneAllowed !== 'all') {
        if (config.filters.plusOneAllowed === 'allowed' && !guest.plus_one) return false
        if (config.filters.plusOneAllowed === 'not_allowed' && guest.plus_one) return false
      }

      // Invitation Sent filter
      if (config.filters.invitationSent !== 'all') {
        if (config.filters.invitationSent === 'sent' && !guest.invitation_sent) return false
        if (config.filters.invitationSent === 'not_sent' && guest.invitation_sent) return false
      }
