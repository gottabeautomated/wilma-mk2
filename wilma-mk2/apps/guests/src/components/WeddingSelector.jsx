...
export const WeddingSelector = () => {
    ...
    const handleSelectWedding = async (weddingId) => {
        try {
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-champagne-50 to-royal-50 p-4">
      <ToastContainer />
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 mb-8">
          {userWeddings.map((wedding) => (<Card key={wedding.id} className="p-6 hover:shadow-lg transition-shadow">
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-lg font-semibold text-royal-900">
                    {wedding.venue_name || 'Neue Hochzeit'}
                  </h3>
                  {currentWedding?.id === wedding.id && (<span className="px-2 py-1 bg-moss-100 text-moss-800 text-xs font-medium rounded-full">
                      Aktiv
                    </span>)}
                </div>
                ...
                <div className="space-y-2 text-sm text-royal-600">
                  ...
                  {wedding.style && (<p>🎨 {wedding.style}</p>)}
                </div>

                <Button onClick={() => handleSelectWedding(wedding.id)} className={`w-full ${currentWedding?.id === wedding.id
          ...
          <Card className="p-6 border-2 border-dashed border-royal-200 hover:border-royal-300 transition-colors">
            <div className="text-center space-y-4">
              ...
              <Button onClick={() => setShowCreateForm(true)} className="w-full bg-champagne-600 hover:bg-champagne-700 text-white">
                Hochzeit erstellen
              </Button>
            </div>
          </Card>
        ...
        {/* Create Wedding Form */}
        {showCreateForm && (<Card className="p-8 max-w-2xl mx-auto">
              ...
              <form onSubmit={handleCreateWedding} className="space-y-4">
                <div className="grid md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <input type="date" value={formData.wedding_date} onChange={(e) => setFormData({ ...formData, wedding_date: e.target.value })} className="w-full px-3 py-2 border border-royal-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-royal-500"/>
                  </div>
                  ...
                  <div>
                    ...
                    <input type="number" value={formData.guest_count} onChange={(e) => setFormData({ ...formData, guest_count: parseInt(e.target.value) })} className="w-full px-3 py-2 border border-royal-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-royal-500"/>
                  </div>
                </div>
                    ...
                    <input type="text" value={formData.venue_name || ''} onChange={(e) => setFormData({ ...formData, venue_name: e.target.value })} placeholder="z.B. Schloss Neuschwanstein" className="w-full px-3 py-2 border border-royal-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-royal-500"/>
                  </div>
                  ...
                  <div>
                    ...
                    <select value={formData.style || ''} onChange={(e) => setFormData({ ...formData, style: e.target.value })} className="w-full px-3 py-2 border border-royal-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-royal-500">
                ...
                <div className="flex gap-3 pt-4">
                  <Button type="button" onClick={() => setShowCreateForm(false)} className="flex-1 bg-gray-200 hover:bg-gray-300 text-gray-700">
                    Abbrechen
                  </Button>
                  <Button type="submit" disabled={loading} className="flex-1 bg-royal-600 hover:bg-royal-700 text-white">
