...
import { Plus, ZoomIn, ZoomOut, RotateCcw, Save, Sparkles, Users } from 'lucide-react';
import { seatingAPI } from '../../lib/supabase';
import TableComponent from './TableComponent';
import AddTableModal from './AddTableModal';
import GuestAssignmentModal from './GuestAssignmentModal';
const SeatingChart = ({ weddingId, readonly = false }) => {
    // State Management
    ...
    const calculateSeatingStats = useCallback(() => {
        ...
        const occupiedTables = tables.filter(t => guests.some(g => g.table_assignment_id === t.id)).length;
        const stats = {
    ...
    useEffect(() => {
        if (!hasUnsavedChanges || readonly || isSaving)
            return;
    ...
    const handleTableDrag = (tableId, info) => {
        if (readonly)
            return;
    ...
    const handleSeatClick = (tableId, seatNumber) => {
        ...
        if (!table)
            return;
        const currentGuest = guests.find(g => g.table_assignment_id === tableId && g.seat_number === seatNumber);
        setAssignmentModal({
            isOpen: true,
            table,
            seatNumber,
    ...
    const handleZoom = (direction) => {
        setChartState(prev => ({
            ...prev,
            zoom: Math.min(MAX_ZOOM, Math.max(MIN_ZOOM, prev.zoom + (direction === 'in' ? 0.2 : -0.2)))
        }));
    };
    const handleReset = () => {
        setChartState(prev => ({
    ...
    return (<div className="bg-white rounded-elegant-lg border border-undertone shadow-elegant overflow-hidden">
      {/* Toolbar */}
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex items-center bg-softrose rounded-elegant border border-undertone">
              <button onClick={() => handleZoom('out')} className="p-2 hover:bg-champagne transition-colors" disabled={chartState.zoom <= MIN_ZOOM}>
              ...
              <button onClick={() => handleZoom('in')} className="p-2 hover:bg-champagne transition-colors" disabled={chartState.zoom >= MAX_ZOOM}>
            ...
            {/* Action Buttons */}
            {!readonly && (<>
                <button onClick={handleReset} className="p-2 bg-softrose hover:bg-champagne rounded-elegant border border-undertone transition-colors" title="Ansicht zurücksetzen">
                  <RotateCcw className="w-4 h-4 text-accent"/>
                </button>
                
                <button onClick={saveChanges} disabled={isSaving} className="flex items-center gap-2 px-4 py-2 bg-royal hover:bg-royal-dark text-white rounded-elegant transition-colors disabled:opacity-50">
        ...
        <div className="flex items-center justify-between mt-3 pt-3 border-t border-undertone">
          ...
          <div className="flex items-center gap-2">
            <label className="flex items-center gap-2 text-sm text-accent cursor-pointer">
              <input type="checkbox" checked={chartState.showRelationships} onChange={(e) => setChartState(prev => ({ ...prev, showRelationships: e.target.checked }))} className="rounded border-undertone text-royal focus:ring-royal"/>
              Beziehungen anzeigen
            </label>
            
            <label className="flex items-center gap-2 text-sm text-accent cursor-pointer">
              <input type="checkbox" checked={chartState.showConflicts} onChange={(e) => setChartState(prev => ({ ...prev, showConflicts: e.target.checked }))} className="rounded border-undertone text-gold focus:ring-gold"/>
              Konflikte anzeigen
            </label>
          </div>
        </div>
      ...
      <div ref={containerRef} className="relative bg-softrose overflow-hidden" style={{ height: '600px' }}>
        <svg ref={svgRef} width="100%" height="100%" viewBox={`0 0 ${VIEWPORT_WIDTH} ${VIEWPORT_HEIGHT}`} className="cursor-grab active:cursor-grabbing">
          {/* Background Grid */}
          <defs>
          ...
          <g transform={`translate(${chartState.pan.x}, ${chartState.pan.y}) scale(${chartState.zoom})`}>
            {/* Render Tables */}
            {tables.map(table => (<TableComponent key={table.id} table={table} guests={guests} isSelected={chartState.selectedTable === table.id} isDragged={chartState.draggedTable === table.id} readonly={readonly} zoom={chartState.zoom} onSelect={handleTableSelect} onDrag={handleTableDrag} onSeatClick={handleSeatClick}/>))}
            
            {/* Render Relationships */}
            {chartState.showRelationships && (<g className="relationships">
        ...
        {/* Floating Add Button */}
        {!readonly && (<motion.button initial={{ scale: 0 }} animate={{ scale: 1 }} whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.95 }} onClick={() => setAddTableModal(true)} className="absolute bottom-6 right-6 w-14 h-14 bg-royal hover:bg-royal-dark text-white rounded-full shadow-royal-lg flex items-center justify-center transition-colors" title="Neuen Tisch hinzufügen">
            <Plus className="w-6 h-6"/>
          </motion.button>)}
      </div>
      
      {/* Add Table Modal */}
      <AddTableModal isOpen={addTableModal} onClose={() => setAddTableModal(false)} onAddTable={handleAddTable} weddingId={weddingId} existingTables={tables}/>
      
      {/* Guest Assignment Modal */}
      {assignmentModal.table && (<GuestAssignmentModal isOpen={assignmentModal.isOpen} onClose={() => setAssignmentModal({ isOpen: false, table: null, seatNumber: 0 })} table={assignmentModal.table} seatNumber={assignmentModal.seatNumber} guests={guests} currentGuestId={assignmentModal.currentGuestId} onAssignGuest={handleAssignGuest} onRemoveGuest={handleRemoveGuest}/>)}
    </div>);
};
