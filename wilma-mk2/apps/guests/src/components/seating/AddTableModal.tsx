...
import { Table, TableType } from '../../types/guest';

interface AddTableModalProps {
  isOpen: boolean;
  onClose: () => void;
  onAddTable: (table: Omit<Table, 'id' | 'created_at' | 'updated_at'>) => void;
  weddingId: string;
...
const AddTableModal: React.FC<AddTableModalProps> = ({
  ...
  const handleSubmit = () => {
    ...
    const newTable: Omit<Table, 'id' | 'created_at' | 'updated_at'> = {
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-white rounded-elegant-lg shadow-royal-xl max-w-md w-full"
          ...
          <div className="p-6 space-y-6">
            ...
            <div>
              ...
              <input
                ...
                placeholder="z.B. Tisch 1, Familientisch, etc."
                className="w-full px-4 py-2 border border-undertone rounded-elegant focus:outline-none focus:ring-2 focus:ring-royal focus:border-transparent"
              />
            </div>
              ...
              <div className="grid grid-cols-3 gap-2">
                {tableTypes.map((type) => {
                  ...
                  return (
                    <button
                      key={type.value}
                      onClick={() => setTableType(type.value as TableType)}
                      className={`p-3 rounded-elegant border-2 transition-all ${
              ...
              <div className="flex gap-2 flex-wrap">
                {colorOptions.map((color) => (
                  <button
                    ...
                    className={`w-10 h-10 rounded-elegant border-2 transition-all ${
                      tableColor === color
                        ? 'border-graphite scale-110'
                        : 'border-undertone hover:scale-105'
                    }`}
            ...
            <div className="bg-softrose p-4 rounded-elegant">
              ...
              <div className="flex items-center justify-center h-20">
                <div className="relative">
                  {tableType === 'round' ? (
                    <div
                      className="w-16 h-16 rounded-full border-2 border-white flex items-center justify-center text-white text-xs font-medium"
                    ...
                    <div
                      className="w-20 h-12 rounded-lg border-2 border-white flex items-center justify-center text-white text-xs font-medium"
                      style={{ backgroundColor: tableColor }}
                    >
                      {tableName || 'Tisch'}
                    </div>
