...
import { Guest, ExportConfig } from '../types/guest.types'
...
export function CSVExportDialog({ isOpen, onClose, guests }: CSVExportDialogProps) {
  ...
  const [selectedPreset, setSelectedPreset] = useState<keyof typeof EXPORT_PRESETS>('basic')
  ...
  const filteredGuests = useMemo(() => {
    return guests.filter(guest => {
      // RSVP Status filter
      if (config.filters.rsvpStatus.length > 0 && !config.filters.rsvpStatus.includes(guest.rsvpStatus)) {
        return false
      }

      // Side filter
      if (config.filters.side.length > 0 && !config.filters.side.includes(guest.side)) {
        return false
      }

      // Table Assignment filter
      if (config.filters.tableAssigned !== 'all') {
        if (config.filters.tableAssigned === 'assigned' && !guest.tableAssignment) return false
        if (config.filters.tableAssigned === 'unassigned' && guest.tableAssignment) return false
      }

      // Plus One filter
      if (config.filters.plusOneAllowed !== 'all') {
        if (config.filters.plusOneAllowed === 'allowed' && !guest.plusOneAllowed) return false
        if (config.filters.plusOneAllowed === 'not_allowed' && guest.plusOneAllowed) return false
      }

      // Invitation Sent filter
      if (config.filters.invitationSent !== 'all') {
        if (config.filters.invitationSent === 'sent' && !guest.invitationSent) return false
        if (config.filters.invitationSent === 'not_sent' && guest.invitationSent) return false
      }
  ...
  const exportStats = useMemo(() => {
    ...
    const withPlusOne = filteredGuests.filter(g => g.plusOneAllowed && g.plusOneRsvp === 'confirmed').length
    const tablesUsed = new Set(filteredGuests.map(g => g.tableAssignment).filter(Boolean)).size
  ...
  const handleFilterChange = (filterType: keyof ExportConfig['filters'], value: any) => {
    setConfig(prev => ({
      ...prev,
      filters: {
        ...prev.filters,
              ...
              {Object.entries(EXPORT_PRESETS).map(([key, preset]) => (
                <Card
                  ...
                  className={`cursor-pointer transition-colors ${
                    selectedPreset === key ? 'ring-2 ring-royal' : ''
                  }`}
                ...
                >
                  <CardHeader className="pb-2">
                    <div className="flex items-center justify-between">
                      ...
                      {selectedPreset === key && (
                        <CheckCircle className="h-5 w-5 text-royal" />
                      )}
                    </div>
                  </CardHeader>
                  <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {['confirmed', 'declined', 'pending', 'maybe'].map(status => (
                    <div key={status} className="flex items-center space-x-2">
                      <Checkbox
                        id={`rsvp-${status}`}
                        checked={config.filters.rsvpStatus.includes(status)}
              ...
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {['bride', 'groom', 'both'].map(side => (
                    <div key={side} className="flex items-center space-x-2">
                      <Checkbox
                        id={`side-${side}`}
                        checked={config.filters.side.includes(side)}
                        onCheckedChange={(checked) => {
              ...
              <div className="space-y-3">
                ...
                <Select
                  value={config.filters.tableAssigned}
                  onValueChange={(value) => handleFilterChange('tableAssigned', value)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="space-y-3">
                ...
                <Select
                  value={config.filters.plusOneAllowed}
                  onValueChange={(value) => handleFilterChange('plusOneAllowed', value)}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
