...
export const WilmaAuthProvider = ({ children }) => {
    const [user, setUser] = useState(null);
    const [session, setSession] = useState(null);
    const [currentWedding, setCurrentWedding] = useState(null);
    const [userWeddings, setUserWeddings] = useState([]);
    ...
    const loadUserWeddings = useCallback(async (userId) => {
        try {
    ...
    const createWedding = async (weddingData) => {
        ...
        const { data, error } = await supabase
            ...
            style: weddingData.style, // Type assertion to match DB enum
