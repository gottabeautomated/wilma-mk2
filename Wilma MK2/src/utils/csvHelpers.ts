...
export function validateGuestData(csvRow: any): ValidationResult {
  const errors: string[] = []
  ...
  if (errors.length === 0) {
    const guest: Guest = {
      ...
      side: csvRow.side as 'bride' | 'groom' | 'both' || 'both',
      rsvpStatus: csvRow.rsvpStatus as 'pending' | 'confirmed' | 'declined' | 'maybe' || 'pending',
      plusOneAllowed: parseBooleanValue(csvRow.plusOneAllowed),
      plusOneName: csvRow.plusOneName?.trim() || undefined,
      plusOneRsvpStatus: csvRow.plusOneRsvpStatus as 'pending' | 'confirmed' | 'declined' | 'maybe' || 'pending',
...
export function generateCSVTemplate(sampleData: any[]): string {
  const headers = [
    'firstName',
    'lastName',
    'email',
...
export function exportGuestsToCSV(guests: Guest[], includeFields?: string[], fileName?: string): void {
  // Feld-Mapping für das neue erweiterte Guest-Interface
  const fieldMap: Record<string, (guest: Guest) => string> = {
...
export function generatePrintableList(guests: Guest[], includeFields: string[], fileName: string): void {
         ...
         <div class="guest">
           ...
           <div class="guest-details">
             ${includeFields.map(field => {
               ...
               switch (field) {
                 case 'firstName': value = guest.firstName; break
                 case 'lastName': value = guest.lastName || ''; break
                 case 'email': value = guest.email || ''; break
                 case 'phone': value = guest.phone || ''; break
                 case 'relationship': value = guest.relationship || ''; break
                 case 'rsvpStatus': 
                   value = guest.rsvpStatus === 'confirmed' ? 'Zugesagt' :
                           guest.rsvpStatus === 'declined' ? 'Abgesagt' :
                           guest.rsvpStatus === 'pending' ? 'Ausstehend' : 'Vielleicht'
                   break
                 case 'dietaryRestrictions': value = guest.dietaryRestrictions || ''; break
                 case 'tableAssignment': value = guest.tableAssignment || guest.tableId || ''; break
                 case 'notes': value = guest.notes || ''; break
                 default: value = ''
               }
