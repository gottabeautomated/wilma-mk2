...
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter, Button, Progress, Badge, Alert, AlertDescription, Table, TableBody, TableCell, TableHead, TableHeader, TableRow, Tabs, TabsContent, TabsList, TabsTrigger, } from './ui';
import { Upload, Download, CheckCircle, AlertTriangle, XCircle, Users, FileSpreadsheet, RefreshCw, AlertCircle, Trash2, FileCheck, } from 'lucide-react';
...
export function CSVImportModal({ isOpen, onClose, onImport, existingGuests }) {
    ...
    const checkForDuplicates = useCallback((guest) => {
        return existingGuests.some(existing => existing.email && guest.email && existing.email.toLowerCase() === guest.email.toLowerCase() ||
    ...
    const getStatusBadge = (status) => {
        const configs = {
            valid: { label: 'Gültig', variant: 'default' },
    ...
    return (<Dialog open={isOpen} onOpenChange={handleClose}>
      <DialogContent className="sm:max-w-6xl max-h-[90vh] overflow-y-auto">
        ...
        <AnimatePresence mode="wait">
          {/* Upload Step */}
          {currentStep === 'upload' && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-6">
              <Alert>
                ...
                <AlertDescription className="flex items-center justify-between">
                  ...
                  <Button variant="outline" size="sm" onClick={downloadTemplate} className="ml-4">
              ...
              <div {...getRootProps()} className={`
                ...
                <div className="space-y-4">
                  ...
                  {isDragActive ? (<p className="text-lg font-medium text-royal">
          ...
          {/* Preview Step */}
          {currentStep === 'preview' && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className="space-y-6">
              ...
              <Tabs defaultValue="all" className="w-full">
                ...
                {['all', 'valid', 'warnings', 'errors', 'duplicates'].map(tab => (<TabsContent key={tab} value={tab} className="mt-4">
                    ...
                    .map((row) => (<TableRow key={row.index}>
                                  <TableCell>
                                    <div className="flex items-center gap-2">
                                  ...
                                  <TableCell>
                                    <div>
                                      ...
                                      {row.data.relationship && (<p className="text-sm text-gray-600">
                        ...
                        row.data.side === 'groom' ? 'Bräutigam' : 'Beide'}
                                  ...
                                  <TableCell>
                                    <div className="space-y-1">
                                      {row.errors.map((error, i) => (<p key={i} className="text-sm text-red-600">• {error}</p>))}
                                      {row.warnings.map((warning, i) => (<p key={i} className="text-sm text-yellow-600">• {warning}</p>))}
                                      {row.status === 'duplicate' && (<p className="text-sm text-blue-600">• Gast bereits vorhanden</p>)}
                                    </div>
                                  </TableCell>
                                  <TableCell>
                                    <Button variant="ghost" size="sm" onClick={() => removeRow(row.index)} className="h-8 w-8 p-0">
          ...
          {/* Complete Step */}
          {currentStep === 'complete' && (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="text-center space-y-6 py-8">
        ...
        <DialogFooter>
          {currentStep === 'upload' && (<Button variant="outline" onClick={handleClose}>
              Abbrechen
            </Button>)}
          ...
          {currentStep === 'preview' && (<>
              ...
              <Button onClick={handleImport} disabled={stats.valid + stats.warnings === 0} className="gap-2">
