    ...
    const filteredGuests = useMemo(() => {
        return guests.filter(guest => {
            ...
            if (config.filters.tableAssigned !== 'all') {
                if (config.filters.tableAssigned === 'assigned' && !guest.tableAssignment)
                    return false;
                if (config.filters.tableAssigned === 'unassigned' && guest.tableAssignment)
                    return false;
            }
            // Plus One filter
            if (config.filters.plusOneAllowed !== 'all') {
                if (config.filters.plusOneAllowed === 'allowed' && !guest.plusOneAllowed)
                    return false;
                if (config.filters.plusOneAllowed === 'not_allowed' && guest.plusOneAllowed)
                    return false;
            }
            // Invitation Sent filter
            if (config.filters.invitationSent !== 'all') {
                if (config.filters.invitationSent === 'sent' && !guest.invitationSent)
                    return false;
                if (config.filters.invitationSent === 'not_sent' && guest.invitationSent)
                    return false;
            }
            return true;
        });
