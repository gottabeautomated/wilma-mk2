...
const StyleForm = ({ data, errors, onChange }) => {
    ...
    return (<motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="max-w-6xl mx-auto space-y-12">
      {/* Elegant Header */}
      ...
      <div className="space-y-12">
        ...
        <ImageSelector options={weddingStyles} selectedValue={data.weddingStyle} onSelect={(value) => updateField('weddingStyle', value)} title="Hochzeitsstil" subtitle="Wählen Sie den Stil, der am besten zu Ihrer Traumhochzeit passt" icon="castle" gridCols="grid-cols-1 md:grid-cols-2 lg:grid-cols-3" imageClassName="aspect-[4/3]"/>

        {errors.weddingStyle && (<motion.p initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="text-red-500 text-sm text-center font-medium">
            {errors.weddingStyle}
          </motion.p>)}

        {/* Color Scheme Section with Images */}
        <ImageSelector options={colorSchemes} selectedValue={data.colorScheme?.[0]} // Get first item from array
     onSelect={selectColorScheme} title="Farbpalette" subtitle="Welche Farben sollen Ihre Hochzeit prägen?" icon="sparkles" gridCols="grid-cols-2 md:grid-cols-3" imageClassName="aspect-[3/2]"/>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="section-elegant">
            ...
            <div className="grid grid-cols-2 gap-3">
              {seasons.map((season, index) => (<motion.button key={season.id} onClick={() => updateField('season', season.id)} className={`card-elegant text-center transition-all duration-300 ${data.season === season.id
                ...
                : 'hover:border-accent/50 hover:bg-primary-50'}`} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 * index }} whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
                  <ElegantIcon name={season.icon} className={`mx-auto mb-2 ${data.season === season.id ? 'text-accent' : 'text-accent/60'}`} size={20}/>
          ...
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }} className="section-elegant">
            ...
            <div className="grid grid-cols-2 gap-3">
              {timesOfDay.map((time, index) => (<motion.button key={time.id} onClick={() => updateField('timeOfDay', time.id)} className={`card-elegant text-center transition-all duration-300 ${data.timeOfDay === time.id
                ? 'border-2 border-accent bg-primary-100 shadow-golden'
                : 'hover:border-accent/50 hover:bg-primary-50'}`} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.1 * index }} whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
                  <ElegantIcon name={time.icon} className={`mx-auto mb-2 ${data.timeOfDay === time.id ? 'text-accent' : 'text-accent/60'}`} size={20}/>
                  <div className="font-serif font-semibold text-graphite text-sm">
                    {time.name}
                  </div>
                  <div className="text-xs text-accent/70 font-medium">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }} className="section-elegant">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {formalityLevels.map((level, index) => (<motion.button key={level.id} onClick={() => updateField('formalityLevel', level.id)} className={`card-elegant text-center transition-all duration-300 ${data.formalityLevel === level.id
                ? 'border-2 border-accent bg-primary-100 shadow-golden'
                ...
                <ElegantIcon name={level.icon} className={`mx-auto mb-3 ${data.formalityLevel === level.id ? 'text-accent' : 'text-accent/60'}`} size={24}/>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.6 }} className="section-elegant">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {culturalTraditions.map((tradition, index) => (<motion.button key={tradition.id} onClick={() => toggleArrayField('culturalTraditions', tradition.id)} className={`card-elegant text-center transition-all duration-300 ${data.culturalTraditions?.includes(tradition.id)
                ? 'border-2 border-accent bg-primary-100 shadow-golden'
                : 'hover:border-accent/50 hover:bg-primary-50'}`} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.05 * index }} whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
                <ElegantIcon name={tradition.icon} className={`mx-auto mb-2 ${data.culturalTraditions?.includes(tradition.id) ? 'text-accent' : 'text-accent/60'}`} size={16}/>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }} className="section-elegant">
          <div className="text-center mb-6">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
            {specialRequirements.map((requirement, index) => (<motion.button key={requirement.id} onClick={() => toggleArrayField('specialRequirements', requirement.id)} className={`card-elegant text-center transition-all duration-300 ${data.specialRequirements?.includes(requirement.id)
                ? 'border-2 border-accent bg-primary-100 shadow-golden'
                : 'hover:border-accent/50 hover:bg-primary-50'}`} initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ delay: 0.05 * index }} whileHover={{ scale: 1.05, y: -2 }} whileTap={{ scale: 0.95 }}>
                <ElegantIcon name={requirement.icon} className={`mx-auto mb-2 ${data.specialRequirements?.includes(requirement.id) ? 'text-accent' : 'text-accent/60'}`} size={16}/>
                <span className="font-medium text-graphite text-xs leading-tight">
                  {requirement.name}
                </span>
              </motion.button>))}
