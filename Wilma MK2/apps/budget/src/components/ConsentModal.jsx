...
export const ConsentModal = ({ isOpen, email, onConsentGiven, onDecline }) => {
    ...
    return (<AnimatePresence>
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 bg-black bg-opacity-50 z-50 flex items-center justify-center p-4">
        <motion.div initial={{ scale: 0.9, opacity: 0, y: 20 }} animate={{ scale: 1, opacity: 1, y: 0 }} exit={{ scale: 0.9, opacity: 0, y: 20 }} className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
          {/* Header */}
          <div className="bg-gradient-to-r from-royal to-royal-light p-6 rounded-t-2xl">
            <div className="flex items-center space-x-3">
              <ElegantIcon name="shield-check" className="w-8 h-8 text-white"/>
              <div>
          ...
          <div className="p-6 space-y-6">
            ...
            <div className="space-y-4">
              ...
              <div className="section-elegant p-4">
                <label className="flex items-start space-x-3 cursor-pointer">
                  <input type="checkbox" checked={consents.aiProcessing} onChange={(e) => setConsents(prev => ({
            ...prev,
            aiProcessing: e.target.checked
              ...
              <div className="section-elegant p-4">
                <label className="flex items-start space-x-3 cursor-pointer">
                  <input type="checkbox" checked={consents.marketing} onChange={(e) => setConsents(prev => ({
            ...prev,
            marketing: e.target.checked
              ...
              <div className="section-elegant p-4">
                <label className="flex items-start space-x-3 cursor-pointer">
                  <input type="checkbox" checked={consents.analytics} onChange={(e) => setConsents(prev => ({
            ...prev,
            ...
            <div className="border-t pt-4">
              <button onClick={() => setShowDetails(!showDetails)} className="flex items-center space-x-2 text-royal hover:text-royal-light text-sm">
                <ElegantIcon name={showDetails ? "chevron-up" : "chevron-down"} className="w-4 h-4"/>
                <span>Detaillierte Datenschutz-Informationen</span>
              </button>

              {showDetails && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="mt-4 p-4 bg-gray-50 rounded-lg text-sm text-gray-600">
            ...
            <div className="flex space-x-4 pt-4">
              ...
              <button onClick={handleConsentSubmit} className="btn-primary flex-1">
