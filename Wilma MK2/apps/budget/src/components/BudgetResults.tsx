...
import { BudgetCalculationResult, budgetService } from '../lib/budgetService'
...
const BudgetResults: React.FC<BudgetResultsProps> = ({ 
  ...
  const getBudgetEfficiencyColor = (efficiency: string) => {
    switch (efficiency) {
  ...
  const getBudgetEfficiencyText = (efficiency: string) => {
    switch (efficiency) {
  ...
  const getBudgetEfficiencyIcon = (efficiency: string) => {
    switch (efficiency) {
  ...
  return (
    <div className="section-elegant">
      ...
      >
        ...
        <div className="text-center space-y-4">
          <motion.div
            ...
            className="w-20 h-20 mx-auto bg-gradient-to-br from-royal via-royal-light to-accent rounded-elegant flex items-center justify-center shadow-royal mb-6"
          >
          ...
          <motion.h1
            ...
            className="text-4xl md:text-5xl font-serif font-bold text-graphite"
          >
            Ihr Hochzeitsbudget
          </motion.h1>
          ...
          <motion.p
            ...
            className="text-xl text-accent font-light"
          >
          ...
          <motion.div
            ...
            className="flex items-center justify-center space-x-2 text-sm text-accent"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          <motion.div
            ...
            className="card-elegant text-center hover:shadow-royal transition-all duration-300"
          >
          ...
          <motion.div
            ...
            className="card-elegant text-center hover:shadow-royal transition-all duration-300"
          >
            <div className={`w-12 h-12 mx-auto mb-4 rounded-elegant flex items-center justify-center bg-gradient-to-br ${
              calculationResult.budgetEfficiency === 'high' ? 'from-green-500 to-green-600' :
          ...
          >
            ...
            <div className="h-80">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie
                    data={calculationResult.categories}
                    cx="50%"
                    cy="50%"
                    outerRadius={100}
                    fill="#8884d8"
                  ...
                  >
                    {calculationResult.categories.map((entry, index) => (
            ...
            <div className="space-y-4 max-h-80 overflow-y-auto">
              {calculationResult.categories.map((category, index) => (
                <motion.div
                  ...
                  className="flex items-center space-x-4 p-3 bg-gradient-to-r from-undertone to-transparent rounded-elegant hover:from-royal/5 hover:to-royal/10 transition-all duration-300"
                >
                  <div 
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            ...
            <div className="space-y-3">
              {calculationResult.recommendations.map((rec, index) => (
                <motion.div
                  ...
                  className="flex items-start space-x-3 p-3 bg-gradient-to-r from-royal/5 to-transparent rounded-elegant"
                >
          ...
          >
            ...
            <div className="space-y-3">
              {calculationResult.savingTips.map((tip, index) => (
                <motion.div
                  key={index}
                  initial={{ opacity: 0, x: -20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: 1.7 + index * 0.1 }}
                  className="flex items-start space-x-3 p-3 bg-gradient-to-r from-green-50 to-transparent rounded-elegant"
                >
        ...
        {(calculationResult.personalizedAdvice?.length > 0 || calculationResult.riskAnalysis?.length > 0) && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
            {/* Personalized Advice */}
            {calculationResult.personalizedAdvice?.length > 0 && (
              ...
              >
                ...
                <div className="space-y-3">
                  {calculationResult.personalizedAdvice.map((advice, index) => (
                    <motion.div
                      key={index}
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: 1.9 + index * 0.1 }}
                      className="flex items-start space-x-3 p-3 bg-gradient-to-r from-accent/10 to-transparent rounded-elegant"
                    >
            ...
            {calculationResult.riskAnalysis?.length > 0 && (
              ...
              >
                ...
                <div className="space-y-3">
                  {calculationResult.riskAnalysis.map((risk, index) => (
                    <motion.div
                      key={index}
                      initial={{ opacity: 0, x: -20 }}
                      animate={{ opacity: 1, x: 0 }}
                      transition={{ delay: 2.1 + index * 0.1 }}
                      className="flex items-start space-x-3 p-3 bg-gradient-to-r from-orange-50 to-transparent rounded-elegant border-l-4 border-orange-200"
                    >
      ...
      {showShareModal && (
        ...
        >
          <motion.div
            className="card-elegant max-w-md w-full"
