...
export class AIService {
    ...
    async generateIntelligentRecommendations(budgetData) {
...
`;
    ...
    validateBudgetReality(budgetData) {
        ...
        const styleLocationMismatch = this.checkStyleLocationMismatch(budgetData.weddingStyle, budgetData.venueType);
        // Budget-Venue-Realität prüfen
        const venueRealityCheck = this.checkVenueBudgetReality(budgetData.venueType, budgetData.totalBudget, budgetData.guestCount);
        return {
            budgetSegment,
    ...
    getBudgetSegmentRealistic(pricePerGuest) {
        if (pricePerGuest < 150)
            return 'Budget-Segment (unter €150/Gast)';
        if (pricePerGuest < 250)
            return 'Standard-Segment (€150-250/Gast)';
        if (pricePerGuest < 400)
            return 'Premium-Segment (€250-400/Gast)';
        return 'Luxus-Segment (€400+/Gast)';
    }
    checkStyleLocationMismatch(style, venueType) {
        const warnings = [];
    ...
    checkVenueBudgetReality(venueType, budget, guests) {
    ...
    generateRealityBasedRecommendations(budgetData) {
        const pricePerGuest = budgetData.totalBudget / budgetData.guestCount;
    ...
    createRealisticPrompt(budgetData, realityCheck) {
...
1. [Budget-Risiken und konkrete Lösungen für ${weddingLocation}]
    ...
    createDetailedAnalysis(budgetData) {
        const pricePerGuest = budgetData.totalBudget / budgetData.guestCount;
    ...
    createComprehensivePrompt(budgetData, analysis) {
        return `
...
...
    ...
    getBudgetSegment(percentile) {
        if (percentile > 90)
            return 'Ultra-Luxus-Segment';
        if (percentile > 75)
            return 'Luxus-Segment';
        if (percentile > 50)
            return 'Gehobenes Segment';
        if (percentile > 25)
            return 'Standard-Segment';
        return 'Budget-Segment';
    }
    getRegionalMultiplier(location) {
        if (!location)
            return 1.0;
    ...
    calculateFeasibilityScore(budgetData, pricePerGuest) {
        let score = 50; // Basis-Score
        // Budget-realismus
        if (pricePerGuest >= 150)
            score += 30;
        else if (pricePerGuest >= 100)
            score += 20;
        else if (pricePerGuest >= 80)
            score += 10;
        else
            score -= 20;
        // Gästeanzahl-realismus
        if (budgetData.guestCount <= 80)
            score += 15;
        else if (budgetData.guestCount <= 120)
            score += 10;
        else
            score -= 10;
        // Flexibilität
        if (budgetData.budgetFlexibility === 'flexible')
            score += 15;
        else if (budgetData.budgetFlexibility === 'strict')
            score -= 10;
        return Math.max(0, Math.min(100, score));
    }
    identifyRiskFactors(budgetData, analysis) {
        const risks = [];
    ...
    generateRuleBasedRecommendations(budgetData) {
        const pricePerGuest = budgetData.totalBudget / budgetData.guestCount;
        const recommendations = [];
        const savingTips = [];
