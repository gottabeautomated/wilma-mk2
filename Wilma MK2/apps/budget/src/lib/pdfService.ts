...
import { BudgetCalculationResult } from './budgetService'

export class PDFService {
  async generatePDF(budgetData: BudgetData, calculationResult: BudgetCalculationResult): Promise<void> {
  ...
  private async renderPageToPDF(pdf: jsPDF, pageElement: HTMLElement, addNewPage: boolean): Promise<void> {
  ...
  private hasAIContent(calculationResult: BudgetCalculationResult): boolean {
    return (
  ...
  private createPage1(budgetData: BudgetData, calculationResult: BudgetCalculationResult): HTMLElement {
  ...
  private createPage2(budgetData: BudgetData, calculationResult: BudgetCalculationResult): HTMLElement {
    const container = document.createElement('div')
    container.style.cssText = `
      position: fixed;
      top: -9999px;
  ...
  private createSummaryCard(icon: string, value: string, label: string, color: string): string {
    return `
      <div style="
        background: rgba(255, 255, 255, 0.9);
        border-radius: 10px;
  ...
  private getEfficiencyIcon(efficiency: string): string {
    switch (efficiency) {
      case 'high': return '💝'
      case 'medium': return '✨'
      case 'low': return '👑'
  ...
  private getEfficiencyText(efficiency: string): string {
    switch (efficiency) {
  ...
  private getEfficiencyColor(efficiency: string): string {
    switch (efficiency) {
