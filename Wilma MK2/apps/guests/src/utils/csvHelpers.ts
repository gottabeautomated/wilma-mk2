...
export function convertValue(value: any, fieldName: string): any {
  ...
  switch (fieldName) {
    ...
    case 'side':
      ...
      if (sideValue.includes('braut') || sideValue === 'bride') return 'bride'
      if (sideValue.includes('bräutigam') || sideValue === 'groom') return 'groom'
      if (sideValue.includes('beide') || sideValue === 'both') return 'both'
      return 'both' // default
      
    case 'rsvpStatus':
      const rsvpValue = String(value).toLowerCase().trim()
      if (rsvpValue.includes('zugesagt') || rsvpValue === 'confirmed' || rsvpValue === 'ja') return 'confirmed'
      if (rsvpValue.includes('abgesagt') || rsvpValue === 'declined' || rsvpValue === 'nein') return 'declined'
      if (rsvpValue.includes('vielleicht') || rsvpValue === 'maybe') return 'maybe'
...
export function validateGuestData(data: any): {
  guest?: Guest
  errors: string[]
  warnings: string[]
} {
  ...
  const exportData = guests.map(guest => {
    ...
    includeFields.forEach(field => {
      switch (field) {
        ...
        case 'phone':
        case 'notes':
          row[field] = guest[field as keyof Guest] || ''
          break
        case 'relationship':
    ...
    includeFields.forEach(field => {
      ...
      switch (field) {
        ...
        case 'tableAssignment':
          ...
          break
        default:
          value = guest[field as keyof Guest] as string || ''
      }
