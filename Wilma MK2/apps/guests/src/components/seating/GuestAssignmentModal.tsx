...
import { Guest, Table } from '../../types/guest';
...
const GuestAssignmentModal: React.FC<GuestAssignmentModalProps> = ({
  isOpen,
  onClose,
  table,
  seatNumber,
  ...
  const [selectedGuestId, setSelectedGuestId] = useState<string | null>(currentGuestId || null);
  
  // Filter available guests (not assigned to any seat)
  const availableGuests = guests.filter(guest => 
    !guest.table_assignment_id || guest.id === currentGuestId
  );
    ...
    <AnimatePresence>
      ...
      >
        <motion.div
          ...
          className="bg-white rounded-elegant-lg shadow-royal-xl max-w-md w-full max-h-[90vh] overflow-hidden"
        ...
        >
          ...
          <div className="border-b border-undertone p-6">
            <div className="flex items-center justify-between">
              ...
              <button
                onClick={onClose}
                className="p-2 text-accent hover:text-graphite hover:bg-softrose rounded-elegant transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="border-b border-undertone p-6">
              <div className="flex items-center justify-between">
                  ...
                  <div className="w-10 h-10 rounded-full bg-accent flex items-center justify-center">
                    {currentGuest.photo_url ? (
                      <img
                        ...
                        alt={`${currentGuest.first_name} ${currentGuest.last_name}`}
                ...
                <button
                  ...
                  className="px-4 py-2 bg-red-100 hover:bg-red-200 text-red-700 rounded-elegant transition-colors"
                >
                  Entfernen
                </button>
              </div>
          ...
          <div className="p-6 border-b border-undertone">
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 border border-undertone rounded-elegant focus:outline-none focus:ring-2 focus:ring-royal focus:border-transparent"
              />
            </div>
          </div>
              ...
              <div className="p-2">
                {filteredGuests.map(guest => (
                  <motion.div
                    ...
                    className={`p-4 m-2 rounded-elegant border cursor-pointer transition-all ${
                      selectedGuestId === guest.id
                        ? 'bg-royal text-white border-royal'
                        : 'bg-white hover:bg-softrose border-undertone'
                    }`}
                    onClick={() => setSelectedGuestId(guest.id)}
                  >
                    <div className="flex items-center gap-3">
                      <div className={`w-10 h-10 rounded-full flex items-center justify-center ${
                        selectedGuestId === guest.id ? 'bg-white bg-opacity-20' : 'bg-accent'
                      }`}>
                        {guest.photo_url ? (
                          <img
                            src={guest.photo_url}
                          ...
                          <span className={`font-medium ${
                            selectedGuestId === guest.id ? 'text-white' : 'text-white'
                          }`}>
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2">
                          ...
                          {guest.relationship_to_couple === 'Braut' || guest.relationship_to_couple === 'Bräutigam' && (
                            <Crown className="w-4 h-4 text-gold" />
                          )}
                        </div>
                        
                        <p className={`text-sm ${
                          selectedGuestId === guest.id ? 'text-white text-opacity-80' : 'text-accent'
                        }`}>
          ...
          <div className="border-t border-undertone p-6">
            <div className="flex justify-end gap-3">
              <button
                onClick={onClose}
                className="px-4 py-2 border border-undertone text-accent hover:bg-softrose rounded-elegant transition-colors"
              >
                Abbrechen
              </button>
              ...
              <button
                ...
                className="px-4 py-2 bg-royal hover:bg-royal-dark text-white rounded-elegant transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Zuweisen
              </button>
            </div>
