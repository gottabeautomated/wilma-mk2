...
import { Plus, Search, Download, Users, MapPin, Phone, Mail, Edit2, Trash2, CheckCircle, XCircle, Clock, User, Heart } from 'lucide-react';
...
export function GuestList({ guests, filters, onFiltersChange, onGuestUpdate, onGuestDelete, onBulkUpdate, onGuestImport }) {
    ...
    const handleGuestSave = async (guestData) => {
        try {
            if (editingGuest) {
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <div className="flex gap-2">
          <Button variant="outline" size="sm" onClick={() => setIsExportDialogOpen(true)} className="border-undertone">
            <Download className="h-4 w-4 mr-2"/>
            Exportieren
          </Button>
          <Button onClick={() => setIsAddModalOpen(true)} size="sm" className="bg-royal hover:bg-royal/90">
      ...
      {/* Bulk Actions */}
      {selectedGuests.length > 0 && (<BulkActions selectedCount={selectedGuests.length} onBulkUpdate={handleBulkUpdate} onClearSelection={() => setSelectedGuests([])}/>)}
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="flex-1">
            <div className="relative">
              ...
              <input type="text" placeholder="Suche nach Name, E-Mail oder Telefon..." value={filters.search} onChange={(e) => onFiltersChange({ ...filters, search: e.target.value })} className="w-full pl-10 pr-4 py-2 border border-undertone rounded-elegant-lg focus:outline-none focus:ring-2 focus:ring-royal/20"/>
            </div>
          </div>
          <div className="flex gap-2">
      ...
      <Card className="overflow-hidden">
        <div className="p-4 border-b border-undertone">
          <div className="flex justify-between items-center">
            <div className="flex items-center gap-2">
              <Checkbox checked={selectedGuests.length === filteredGuests.length && filteredGuests.length > 0} onCheckedChange={handleSelectAll}/>
        ...
        <div className="overflow-x-auto">
          <Table>
            ...
            <TableBody>
              {filteredGuests.map((guest) => (<TableRow key={guest.id}>
                  <TableCell>
                    <Checkbox checked={selectedGuests.includes(guest.id)} onCheckedChange={(checked) => handleGuestSelection(guest.id, checked)}/>
                  </TableCell>
                  <TableCell>
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      {guest.email && (<div className="flex items-center gap-1 text-sm">
                          <Mail className="h-3 w-3"/>
                          {guest.email}
                        </div>)}
                      {guest.phone && (<div className="flex items-center gap-1 text-sm">
                  ...
                  <TableCell>
                    {guest.table_assignment ? (<div className="flex items-center gap-1">
                        <MapPin className="h-3 w-3"/>
                        Tisch {guest.table_assignment}
                        {guest.seat_number && ` (${guest.seat_number})`}
                      </div>) : (<span className="text-undertone">Nicht zugewiesen</span>)}
                  </TableCell>
                  <TableCell>
                    {guest.plus_one ? (<Badge variant="secondary" className="bg-royal/10 text-royal">
                        Erlaubt
                  ...
                  <TableCell>
                    <div className="flex items-center gap-1">
                      <Button variant="ghost" size="sm" onClick={() => handleGuestEdit(guest)} className="h-8 w-8 p-0">
                        <Edit2 className="h-4 w-4"/>
                      </Button>
                      <Button variant="ghost" size="sm" onClick={() => onGuestDelete(guest.id)} className="h-8 w-8 p-0 text-red-500 hover:text-red-700">
      ...
      {/* Modals */}
      <AddGuestModal isOpen={isAddModalOpen} onClose={() => {
            setIsAddModalOpen(false);
            setEditingGuest(null);
        }} onSave={handleGuestSave} existingGuest={editingGuest}/>
      
      <CSVExportDialog isOpen={isExportDialogOpen} onClose={() => setIsExportDialogOpen(false)} guests={filteredGuests}/>
    </div>);
}
