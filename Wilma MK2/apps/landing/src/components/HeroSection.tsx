        ...
        <div className="hero-tools">
          ...
          {tools.map((tool, index) => (
            <Link 
              key={index}
              href={tool.url}
              className={`tool-card ${currentTool === index ? 'active' : ''}`}
              style={{ '--tool-color': `linear-gradient(135deg, ${tool.color.split(' ')[1]}, ${tool.color.split(' ')[3]})` } as React.CSSProperties}
              onMouseEnter={() => setCurrentTool(index)}
            >
