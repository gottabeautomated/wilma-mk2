      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur shadow flex items-center justify-between px-6 py-3">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button onClick={() => setLoginOpen(!loginOpen)} className="bg-gradient-to-r from-pink-600 to-purple-600 text-white px-4 py-2 rounded-full text-sm font-semibold hover:scale-105 transition-transform duration-200 flex items-center gap-2">
              <span>🔑</span>
              <span>Login</span>
            </button>
            {/* Login Dropdown */}
            {loginOpen && (<div className="absolute top-full right-0 mt-2 w-80 bg-white rounded-2xl shadow-2xl border border-gray-100 p-6 animate-fadeIn">
                <div className="flex justify-between items-center mb-4">
                  <h3 className="font-serif text-lg font-semibold text-gray-800">Willkommen zurück</h3>
                  <button onClick={() => setLoginOpen(false)} className="text-gray-400 hover:text-gray-600 text-xl">
                    ×
                  </button>
                </div>
                <form className="space-y-4">
                  <div>
                    ...
                    <input type="email" className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent" placeholder="ihre.email@beispiel.at" required/>
                  </div>
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Passwort</label>
                    <div className="relative">
                      <input type={showPassword ? "text" : "password"} className="w-full px-3 py-2 pr-10 border border-gray-300 rounded-lg focus:ring-2 focus:ring-pink-500 focus:border-transparent" placeholder="Ihr Passwort" required/>
                  ...
                  <button type="submit" className="w-full bg-gradient-to-r from-pink-600 to-purple-600 text-white py-2 rounded-lg font-semibold hover:scale-105 transition-transform duration-200">
                    🔑 Anmelden
                  </button>
          ...
          {/* Menu Button */}
          <button className="flex flex-col justify-center items-center w-10 h-10 group" aria-label="Menü öffnen" onClick={() => setMenuOpen((v) => !v)}>
        ...
        {/* Overlay-Menü */}
        {menuOpen && (<div className="fixed inset-0 bg-white/95 backdrop-blur z-50 flex flex-col items-center justify-start pt-32 animate-fadeIn">
